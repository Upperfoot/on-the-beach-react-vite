...
describe("Holiday API Service", () => {
  ...
  test("fetch all hotels, should return a list of hotels on success", async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith("https://static.onthebeach.co.uk/fe-code-test/data.json");
  });
  ...
  test("fetch all hotels, should return an empty array when the API fails", async () => {
    ...
    expect(hotels).toEqual([]); // Should return empty array on failure
    expect(axios.get).toHaveBeenCalledTimes(1);
  });
});